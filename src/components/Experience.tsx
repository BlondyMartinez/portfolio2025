          ...
          <div className="group">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {(t('experience.openSource.items', { returnObjects: true }) as Array<any>).map((item, index) => (
                <div 
                  key={index}
                  className="bg-gray-800/50 backdrop-blur-sm p-8 rounded-xl shadow-2xl border border-gray-700/50 hover:border-blue-400/50 transition-all duration-300"
                  onMouseEnter={() => item.project === 'Official Website of Lola Lolita Land' && setExpandedProjects((prev) => ({ ...prev, [item.project]: true }))}
                  onMouseLeave={() => item.project === 'Official Website of Lola Lolita Land' && setExpandedProjects((prev) => ({ ...prev, [item.project]: false }))}
                >
